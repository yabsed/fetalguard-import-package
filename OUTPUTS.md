...
현장에서 사람이 결과를 읽을 때는 **`internal/onsite_figures/index.html`**부터 연다. 질문별로 관련 지표를 묶은 그래프와 해설이 있으며, PNG/PDF를 직접 열 수도 있다. 세부 수치·개별 사례는 연결된 기존 보고서에서 확인한다.
...
│   ├── onsite_figures/           # 현장 이해용: 질문별 그래프·해설
...
1. `status.json`이 `complete`인지 확인하고 `internal/onsite_figures/index.html`을 연다. 세부 표는 `internal/report/report.html`에서 확인한다.
...
`internal/onsite_figures/`는 사람의 질문 순서로 읽는 자료다. 전체 결과를 지표마다 분할하는 대신 비교에 필요한 결과를 같은 그림에 배치한다.
...
기본 출력은 **`full-실행해시/internal/onsite_figures/`**다. 이전 단계 파일·완료 해시는 유지하며, 읽은 입력과 새 산출물의 해시를 자체 `manifest.json`에 기록한다. 새 파이프라인 실행에서는 별도 단계로 생성·재개 검증하고 `LATEST.json`에 시작 페이지 경로를 남긴다. 개별 구간 SHAP·실제 범위·원 사이트 코드가 포함되므로 현장 전용으로 보존한다.
