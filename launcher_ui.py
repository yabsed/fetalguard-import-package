...
def launch():
    ...
    def run(check_only=False):
        ...
        def worker():
            try:
                ...
                code = process.wait()
                status.value = "완료 — 로그 마지막 줄의 결과 경로를 여세요." if code == 0 else f"중단/실패 (exit {code}). 마지막 오류를 확인하세요. 완료된 학습은 재실행 시 재사용됩니다."
    ...
    display(widgets.VBox([widgets.HTML("<h3>CTG 본선 원클릭 실행</h3><p>코드 수정 없이 경로와 모드만 입력합니다. 결과는 반입 폴더 밖에 저장합니다.</p>"),
        data, output, widgets.HBox([profile, device]), widgets.HBox([check, start, stop]), status, log]))
