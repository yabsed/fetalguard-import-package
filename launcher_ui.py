...
def launch():
    ...
    survey = widgets.Button(description="데이터 조사만", button_style="info")
    ...
    def run(check_only=False, survey_only=False):
        if not data.value.strip() or not output.value.strip():
            status.value = "데이터 폴더와 결과 폴더를 모두 입력하세요."
            return
        start.disabled = check.disabled = survey.disabled = True
        stop.disabled = False
        ...
        def worker():
            try:
                ...
                active["process"] = None
                start.disabled = check.disabled = survey.disabled = False
                stop.disabled = True
    ...
    survey.on_click(lambda _: run(survey_only=True))
    ...
    display(widgets.VBox([widgets.HTML("<h3>CTG 본선 원클릭 실행 · 설계 2.0</h3><p>코드 수정 없이 경로와 모드만 입력합니다. 현장 결과와 반출 심사용 집계 묶음을 별도로 생성합니다.</p>"),
        data, output, widgets.HBox([profile, device]), widgets.HBox([check, start, stop, survey]), prior, status, log]))
