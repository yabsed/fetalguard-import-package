...
### 현장 연속 실행 (best-effort)

`data`부터는 개별 레코드·모델·분석 실패를 내부 `issues.jsonl`에 기록하고 독립적인 다음 작업을 계속한다.
`5 Abnormality labels for 4 selected windows`처럼 정렬을 확인할 수 없는 기록은 **전체 레코드를 제외**하며 라벨을 자르거나 정답을 추정하지 않는다.
제외 목록은 `internal/data/record_exclusions.csv`, 전체 단계별 결과는 `internal/pipeline_status.json`에 있다.
레코드 중간 실패 시 이미 계산한 해당 레코드의 특징·신호도 함께 취소하여 CSV와 NPY 행 정렬을 유지한다.

일부 미산출·제외가 있으면 최종 `status.json`은 `complete_with_issues`이다. 이는 전체 성공이나 연구 타당성 보장이 아니다.
유효 데이터·산모·클래스가 부족하면 학습은 미산출로 남기고 가능한 조사·보고서·심사 이미지를 만든다.
CV/seed 탐색이 불완전하면 이를 완전한 비교 결과로 보고하지 않는다. 반출 집계의 소수 셀 억제·기관 승인 조건은 그대로 유지한다.
`tools/validate_run.py`는 부분 결과에 `PARTIAL_VALIDATED`를 반환하며 검증한 범위를 나열한다.

기존 실행 명령을 그대로 쓴다. 수정본은 패키지 해시가 달라 새 실행 폴더를 만든다.
부분 완료한 단계도 체크포인트로 보존된다. 같은 명령 재실행은 그 결과를 재사용하며, 실패한 작업까지 새로 시도하려면 새 `--output` 경로를 사용한다.
Ctrl+C, 저장 권한 부족, 디스크 용량 부족, 완료 산출물 변조는 여전히 중단한다. 강제 종료·전원 차단까지 실행 완료를 보장하지 않는다.
...
다태아는 ID의 `_1`/`_2`와 `twin1`/`twin2`를 대응시킨다. FHR 결측률이 30%를 초과한 구간은 제외한다. 표본 간격/길이, 라벨 정렬, 산모 ID가 계약과 다르면 오류를 기록하고 해당 레코드를 제외한다. 임의로 라벨이나 산모 ID를 맞추지 않는다.
